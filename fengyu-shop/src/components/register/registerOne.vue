...
export default {
    ...
    setup(){
        ...
        let rules = reactive({
            ...
            phone: [
                { required: true, message: "请设置电话号码", trigger: "blur" },
        ...
        let onRegister=() => {
            formData.value.validate(async(valid) => {
                ...
                if (!valid) {
                    ...
                    const { data: res } = await axios.post("/shopping/register",{name:user.name,password:user.password,account:account,address:user.address,phone:user.phone});
                    console.log(res);
                    //清空表单
                    clearData();
                    ElMessageBox.confirm(
