...
<h4 class="listname">管理员用户管理</h4>
<button class="adduser"><a href="register">添加新用户</a></button>
  <table>
    <thead>
      <tr>
        <th>#</th>
    ...
    <tbody v-if="lookup()">
      <tr v-for="(item,index) in lookup()" :key="item.id">
        ...
        <td>{{item.time}}</td>
        <td>
            <router-link :to="`/main/user/userDetial/${item.id}`" class="linkstyle ctrol-left" >详情</router-link>        
        </td>
        <td>
          <router-link :to="`/main/user/update/${item.id}`" class="linkstyle ctrol-left" >修改</router-link>
          <button  type="button" class="ctrol ctrol-right" @click="userDelete(item.id)">删除</button>
        </td>
      </tr>
    </tbody>
    <tbody v-if="str==null">
...
export default {
  ...
  setup(){
    ...
    async function deleteResult(){
      const {data:res}=await axios.post("/shopping/del",{'id':did.value})
      console.log(res)
      if(res.code==200){
